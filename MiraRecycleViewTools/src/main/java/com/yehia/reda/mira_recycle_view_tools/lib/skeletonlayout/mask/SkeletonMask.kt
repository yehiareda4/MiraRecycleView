...
import com.yehia.reda.mira_recycle_view_tools.lib.skeletonlayout.tag
import com.yehia.reda.mira_recycle_view_tools.lib.skeletonlayout.views
...
internal abstract class SkeletonMask(protected val parent: View, @ColorInt color: Int) :
    ...
    private fun validate(view: View) {
        when (view) {
            is RecyclerView, is ViewPager2 -> Log.w(
                tag(),
                "Passing ViewGroup with reusable children to SkeletonLayout - consider using applySkeleton() instead"
            )
        }
    }
}